...
export class AddProductComponent implements OnInit {
  ...
  constructor(private fb: FormBuilder, private sessionStorageService : SessionStorageService,
  ...
  async submitForm() {
    if (this.validateForm.valid) {
      ...
      if(this.newProduct.sentType === 1){
        this.newProduct.price = this.validateForm.value.price
      }else{
        this.newProduct.price = 0
      }
      this.newProduct.sellerId = this.sessionStorageService.getFromSessionStorage('user').id
